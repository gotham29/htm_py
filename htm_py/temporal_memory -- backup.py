    ...
    def _predict_cells(self):
        ...
        for cell in range(self.numCells):
            for segment in self.connections.segmentsForCell(cell):
                ...
                # ✅ Require that connected presynaptic sources match context
                if connected_sources == self.prevWinnerCells:
                    overlap = len(connected_sources & self.prevActiveCells)
                    if overlap >= self.activationThreshold:
                        self.predictiveCells.add(cell)
                        self.predictedSegmentForCell[cell] = segment
                        logger.debug(
                            f"[PREDICT-CONTEXT] cell {cell} ← segment {segment}, "

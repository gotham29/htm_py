...
class TestGetBestMatchingSegment(unittest.TestCase):
    ...
    def test_returns_segment_with_highest_overlap(self):
        ...
        best = getBestMatchingSegment(self.conn, self.cell, {2, 3, 4}, minThreshold=2, connectedPermanence=0.5)
    ...
    def test_returns_none_if_no_segment_meets_threshold(self):
        ...
        self.conn.createSynapse(seg1, 1, 0.5)
        self.conn.createSynapse(seg1, 2, 0.5)
        result = getBestMatchingSegment(self.conn, self.cell, {3}, minThreshold=2, connectedPermanence=0.5)
    ...
    def test_exactly_at_threshold_returns_segment(self):
        ...
        result = getBestMatchingSegment(self.conn, self.cell, {1, 2}, minThreshold=2, connectedPermanence=0.5)
    ...
    def test_multiple_segments_same_overlap_prefers_first(self):
        ...
        result = getBestMatchingSegment(self.conn, self.cell, {1}, minThreshold=1, connectedPermanence=0.5)
        self.assertIn(result, (seg1, seg2))  # deterministic preference optional

    def test_empty_active_presynaptic_returns_none(self):
        seg = self.conn.createSegment(self.cell)
        self.conn.createSynapse(seg, 2, 0.5)
        result = getBestMatchingSegment(self.conn, self.cell, set(), minThreshold=1, connectedPermanence=0.5)
        self.assertIsNone(result)

    def test_invalid_active_set_raises(self):
        seg = self.conn.createSegment(self.cell)
        with self.assertRaises(TypeError):
            getBestMatchingSegment(self.conn, self.cell, [1, 2], 1, connectedPermanence=0.5)
    ...
    def test_segment_with_zero_synapses_does_not_match(self):
        ...
        result = getBestMatchingSegment(self.conn, self.cell, {1, 2, 3}, minThreshold=1, connectedPermanence=0.5)
        self.assertIsNone(result)
    ...
    def test_multiple_synapses_same_presynaptic_cell_all_count(self):
        ...
        result = getBestMatchingSegment(self.conn, self.cell, {2}, minThreshold=2, connectedPermanence=0.5)
        self.assertEqual(result, seg)
    ...
    def test_segments_on_other_cells_are_ignored(self):
        ...
        result = getBestMatchingSegment(self.conn, self.cell, {2}, minThreshold=1, connectedPermanence=0.5)
        self.assertIsNone(result)
    ...
    def test_many_segments_only_one_matches(self):
        ...
        result = getBestMatchingSegment(self.conn, self.cell, {7}, minThreshold=1, connectedPermanence=0.5)

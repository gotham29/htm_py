...
class TestTemporalMemoryBehavior(unittest.TestCase):
    ...
    def test_column_burst_activates_all_cells(self):
        col = 2
        expected_cells = self.tm.cells_for_column(col)
    ...
    def test_predicted_cells_are_winners(self):
        # Make sure the connected permanence threshold allows 0.5 to count as connected
        self.tm.connected_permanence = 0.5
        self.tm.min_threshold = 2  # ensure that 3 connected synapses pass this

        prev_cells = [0, 1, 2]
        cell = self.tm.cells_for_column(1)[0]
        seg = self.tm.connections.create_segment(cell)

        for c in prev_cells:
            self.tm.connections.create_synapse(seg, c, 0.5)  # exactly at threshold

        # Set active cells manually to simulate previous timestep
        self.tm.active_cells = prev_cells
        self.tm.compute([1], learn=False)

        # Now the segment should be matching and predictive
    ...
    def test_one_winner_per_burst_column(self):
        ...
        self.assertIn(winners[0], self.tm.cells_for_column(col))
    ...
    def test_no_segment_without_winner_cells(self):
        ...
        for c in self.tm.cells_for_column(col):
